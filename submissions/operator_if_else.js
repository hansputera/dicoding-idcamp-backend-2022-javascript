...
/**
 * Score checker.
 * @param {number} score score number to check
 * @return {string}
 */
function scoreChecker(score) {
	let result = 'Anda mendapatkan nilai ';

	if (score >= 90) result = 'Selamat! '.concat(result, 'A.');
	else if (score >= 80 && score <= 89) result = result.concat('B.');
	else if (score >= 70 && score <= 79) result = result.concat('C.');
	else if (score >= 60 && score <= 69) result = result.concat('D.');
